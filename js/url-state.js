...
(function (global) {
  ...
  function readFromUrl() {
    ...
    // Recently-funded toggle (boolean, class-driven — not a plain value field)
    const fundedEl = hooks.getElement('fundedBtn');
    if (fundedEl && sp.get('funded') === '1') {
      fundedEl.classList.add('is-active');
      fundedEl.setAttribute('aria-pressed', 'true');
    }
  ...
  function replaceFromCriteria(criteria) {
    ...
    if (criteria.funded) sp.set('funded', '1');

    const qs = sp.toString();
    const base = qs ? location.pathname + '?' + qs : location.pathname;
    // Preserve the tab hash (#analytics, #about, …) — tab routing owns location.hash.
