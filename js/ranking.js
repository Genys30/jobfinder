...
(function (global) {
  ...
  /**
   * A→Z by employer. Hebrew and Latin names live in the same list, so use a
   * he+en collator (base sensitivity) rather than raw code-point order.
   * Blank companies always sink to the bottom, in both directions.
   */
  const companyCollator = (typeof Intl !== 'undefined' && Intl.Collator)
    ? new Intl.Collator(['he', 'en'], { sensitivity: 'base', numeric: true })
    : null;

  function sortByCompany(jobs, parseDateFn, desc) {
    if (!jobs || !jobs.length) return jobs;
    const sign = desc ? -1 : 1;
    jobs.sort(function (a, b) {
      const ca = String((a && a.company) || '').trim();
      const cb = String((b && b.company) || '').trim();
      if (!ca && !cb) return 0;
      if (!ca) return 1;
      if (!cb) return -1;
      const cmp = companyCollator ? companyCollator.compare(ca, cb) : ca.localeCompare(cb);
      if (cmp) return sign * cmp;
      // Same employer → newest first inside the block
      if (!parseDateFn) return 0;
      const da = parseDateFn(a.updated);
      const db = parseDateFn(b.updated);
      if (!da && !db) return 0;
      if (!da) return 1;
  ...
  function applyResultSort(jobs, opts) {
    ...
    if (sort === 'company-asc' || sort === 'company-desc') {
      return sortByCompany(jobs, parseDateFn, sort === 'company-desc');
    }
    if (!q || sort === 'date') {
