...
(function () {
  ...
  AC.auth.getSession().then(function (res) {
    ...
    setExportVisible(isOwner(user));
  });
  AC.auth.onAuthStateChange(function (_e, session) {
    var owner = session && isOwner(session.user);
    if (owner) injectButton();
    else removeButton();
    setExportVisible(owner);
  });

  // The Jobs "Export CSV" button is owner-only — it dumps the full filtered
  // dataset (the private scraping moat). Hidden by default in index.html; shown
  // here only when the signed-in account is the owner.
  ...
  async function renderOverview(body) {
    try {
      ...
      // All five hand-saved artifacts, not just NLI/NTA: Phoenix, Ayalon and Assuta became
      // manual sources in Aug 2026 and were never given a card, which is why they sat
      // 10 days stale unnoticed. push_health now measures the age from the last commit
      // that changed the file, not its mtime (run_fetch.bat’s git reset --hard rewrites
      // mtime nightly, so these cards used to read "4d old" for 16-day-old data).
      var manualCards = [
        { key: "nli",     label: "NLI",     file: "nli_career.html" },
        { key: "nta",     label: "NTA",     file: "nta_career.html" },
        { key: "phoenix", label: "Phoenix", file: "phoenix_career.html" },
        { key: "ayalon",  label: "Ayalon",  file: "ayalon_career.json" },
        { key: "assuta",  label: "Assuta",  file: "assuta_career.json" }
      ].map(function (m) {
        // Two clocks: data age = last commit that CHANGED the artifact; checked age =
        // last time it was re-saved by hand (refresh_manual writes it even when the
        // re-saved page was identical). We rank the card by the checked age — that is
        // what the operator controls — so a Saturday refresh reads "checked today" even
        // if the board hasn't moved, while a source nobody re-saves still goes stale.
        var checked = meta[m.key + "_checked_days"];
        var age = meta[m.key + "_age_days"];
        var big, val;
        if (checked != null) {
          big = checked;
          val = checked === 0 ? "checked today" : checked + "d since save";
        } else {
          big = (age || 0);
          val = age != null ? age + "d old" : "—";
        }
        var sub = "re-saved by hand · reminder emails on Saturday" +
          (age != null ? " · data " + age + "d old" : "");
        return stage(m.label + " freshness", val, sub,
          big > 8 ? "WARN" : "OK",
          m.label + " hasn’t been re-saved in over a week — re-save " + m.file +
          ", then Actions › “Refresh manual (all 5)”.");
      }).join("");
      ...
      var cards = [
        ...
        stage("Manual sources", man.fresh + "/" + man.total + " fresh today", "LinkedIn (NLI/NTA have their own cards)",
          man.fresh === man.total ? "OK" : "WARN",
          "Drop today’s LinkedIn export, then refresh."),
  ...
  function renderActions(body) {
    body.innerHTML =
      ...
      locBtn("refresh_manual", "Refresh manual (all 5)") +
      locBtn("refresh_nli", "Refresh NLI") +
      locBtn("refresh_nta", "Refresh NTA") +
      locBtn("refresh_phoenix", "Refresh Phoenix") +
      locBtn("refresh_ayalon", "Refresh Ayalon") +
      locBtn("refresh_assuta", "Refresh Assuta") +
      locBtn("refresh_linkedin", "Refresh LinkedIn") +
    ...
    Array.prototype.forEach.call(body.querySelectorAll("[data-cmd]"), function (b) {
      b.onclick = async function () {
        ...
        if (b.dataset.cmd === "refresh_manual" &&
            !confirm("Refresh the hand-saved sources? Save whichever pages you have (NLI, NTA, " +
                     "Phoenix, Ayalon, Assuta) into the repo folder FIRST — this parses every one it " +
                     "finds, rebuilds the bundle once and publishes (no scrape). Sources you did " +
                     "not re-save keep the data they already have. Continue?")) return;
        if (b.dataset.cmd === "refresh_linkedin" &&
            !confirm("Refresh LinkedIn from a manual export? Export today's jobs via the Chrome " +
                     "extension into the repo folder FIRST — this cleans that file, rebuilds the " +
                     "bundle and publishes it (no scrape). Continue?")) return;
        if (b.dataset.cmd === "refresh_phoenix" &&
            !confirm("Refresh Phoenix from a saved page? Save fnx.co.il/career/open-positions as " +
                     "phoenix_career.html in the repo folder FIRST (Ctrl+S, HTML only) — this parses " +
                     "it, rebuilds the bundle and publishes it (no scrape). Continue?")) return;
        if (b.dataset.cmd === "refresh_ayalon" &&
            !confirm("Refresh Ayalon from a saved response? In the careers site open F12 › Network, " +
                     "find the careers POST, Copy › Copy response into ayalon_career.json in the repo " +
                     "folder FIRST — this parses it, rebuilds the bundle and publishes it (no scrape). " +
                     "Continue?")) return;
        if (b.dataset.cmd === "refresh_assuta" &&
            !confirm("Refresh Assuta from a saved response? On the jobs search page open F12 › Network, " +
                     "find the get-jobs request, Copy › Copy response into assuta_career.json in the repo " +
                     "folder FIRST — this parses it, rebuilds the bundle and publishes it (no scrape). " +
                     "Continue?")) return;
