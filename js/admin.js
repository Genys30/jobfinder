...
(function () {
  ...
  async function renderOverview(body) {
    try {
      ...
      // All four hand-saved artifacts, not just NLI/NTA: Phoenix and Ayalon became
      ...
      var manualCards = [
        ...
        return stage(m.label + " freshness", age != null ? age + "d old" : "—",
          "re-saved by hand · reminder emails on Saturday",
          (age || 0) > 8 ? "WARN" : "OK",
          m.label + " is over a week old — re-save " + m.file +
          ", then Actions › “Refresh manual (all 4)”.");
      }).join("");
  ...
  function renderActions(body) {
    body.innerHTML =
      ...
      locBtn("refresh_manual", "Refresh manual (all 4)") +
      ...
      locBtn("sync_new_companies", "Sync new companies") +
      ...
      b.onclick = async function () {
        ...
        if (b.dataset.cmd === "refresh_manual" &&
            !confirm("Refresh the hand-saved sources? Save whichever pages you have (NLI, NTA, " +
                     "Phoenix, Ayalon) into the repo folder FIRST — this parses every one it " +
        ...
        if (b.dataset.cmd === "refresh_ayalon" &&
            !confirm("Refresh Ayalon from a saved response? In the careers site open F12 › Network, " +
                     ...
                     "Continue?")) return;
        if (b.dataset.cmd === "sync_new_companies" &&
            !confirm("Sync the New companies watchlist? If the discovery app isn't running it is " +
                     "started automatically (a window opens) and left running. This pushes its " +
                     "watchlist into the private mirror and syncs your tab edits back. Continue?")) return;
