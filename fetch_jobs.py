...
def run_mod():
    ...
    try:
        ...
        data = r.json()
        if data.get("HasError"): print(f"  x API error: {data.get(chr(77)+chr(101)+chr(115)+chr(115)+chr(97)+chr(103)+chr(101)+chr(115))}"); return
        ...
        for t in tenders:
            ...
            jobs.append({
                ...
                "date_posted": fmt(pub.get("StartDate") or t.get("CreatedAt")),
                "deadline":    fmt(t.get("NomineesApplyingDate")),
                "url":         f"https://jobs.mod.gov.il/#/Tenders?TenderId={tid}",
                "source":      "mod",
                "education":   clean(hj.get("Education") or ""),
                "experience":  clean(hj.get("Experience") or ""),
                "summary":     clean(hj.get("GeneralSummary") or ""),
            })
        print(f"  + {len(jobs)}")
        write_csv(jobs, ["title","company","location","date_posted","deadline","url","source","education","experience","summary"],
